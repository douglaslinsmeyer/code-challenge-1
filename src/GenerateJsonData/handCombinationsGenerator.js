...
const handCombinationsGenerator = function(twoCards) {
	const deck = [
        cards,
        cards,
        cards
    ...
    fs.writeFile(`HandCombinations/${twoCards}.json`, JSON.stringify({combinations}), function(err) {
        if (err) throw err;
        console.log(`${twoCards} Saved`);
    });
    //return combinations;
};
